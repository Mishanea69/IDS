...
int main(int argc, char** argv) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <input.pcap|pcapng> <rules.txt> <alerts.csv>\n";
        std::cerr << "Rule format: <sid> <proto> [src_port=N] [dst_port=N] [nocase] (ascii|hex) \"<pattern>\" [msg=\"...\"]\n";
        return 1;
    }

    const std::string in_pcap = argv[1];
    ...
    if (!p) {
        ...
        std::cerr << "Unsupported datalink type: " << dlt << " (supported: Ethernet=1, SLL=113)\n";
        pcap_close(p);
        return 1;
        }
    ...
    std::unordered_map<AlertKey, double, AlertKeyHash> alert_last_ts;
    alert_last_ts.reserve(200000);
    const double alert_window_s = 60.0; // re-alert after 60s per flow

    std::unordered_map<ThKey, std::deque<double>, ThKeyHash> th_hist;
    std::unordered_map<ThKey, double, ThKeyHash> th_last_alert;
    ...
    while (true) {
        ...
        if (hdr->caplen < l4_payload_off) continue;
        ...
        for (const auto& r : rules) {
            ...
            double last = th_last_alert.count(k) ? th_last_alert[k] : -1e18;
            ...
            if (it != alert_last_ts.end() && (ts - it->second) < alert_window_s) continue;
            alert_last_ts[ak] = ts;
    ...
    pcap_close(p);
    std::cerr << "Processed " << pkt_count << " packets. Wrote " << alert_count << " alerts to " << out_csv << "\n";
    return 0;
}